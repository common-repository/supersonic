...
function wpss_log( $event, $info = '', $ip = null, $ip_country = null ) {
	 ...
	 * 11 - challenge from comment
	 * 14 - ip ban from log
	 * 17 - ip wl from log
	 * 18 - ip nul from log
	 * 21 - challenge from log
	 * 20 - cron job
	 */
	...
	if ( $settings['event_log'] == '1' ) {
		...
		global $wpdb;
		$wpdb->insert( $wpdb->prefix . "wpss_log", array (
				'event' => $event,
...
class WPSS_Log_List_Table extends WP_List_Table {
	...
	function column_time( $item ) {
		...
		return sprintf( '%s %s', $time, $this->row_actions( $actions ) );
	}
	...
	function column_ip_country( $item ) {
		...
		if ( isset( $wpss_countries[$ip_country] ) ) {
			...
			$ret .= '<span> | <a title="Challenge IP in CloudFlare" href="javascript:void(0);" onclick="wpss_ip_action(\'' . $ip . '\',\'' . $ip_country . '\',\'challenge\',\'' . wp_create_nonce( 'wpss_ip_nonce' ) . '\',' . $id . ',\'log\');" class="delete">CH</a></span>';
	...
	function prepare_items() {
		...
		$this->items = $wpdb->get_results( $wpdb->prepare( "SELECT id, time, event, ip, ip_country, info,
        case
          ...
          when event = 7 then 'IP whitelisted from comment'
          when event = 8 then 'IP delete rule from comment'
          when event = 9 then 'Purge CloudFlare cache'
          when event = 10 then 'Report comment spam'
		  when event = 11 then 'IP challenge from comment'
          when event = 14 then 'IP ban from log view'
          when event = 17 then 'IP whitelisted from log view'
          when event = 18 then 'IP delete from log view'
          when event = 20 then 'Cron job'
		  when event = 11 then 'IP challenge from log view'
          else 'Undefined'
